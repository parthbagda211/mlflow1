...
if __name__ == "__main__":
    ...
    with mlflow.start_run():
        ...
        for epoch in range(int(epochs)):
            ...
            for x_batch,y_batch in train_dataset:
                with tf.GradientTape() as tape:
                    y_pred_batch = l_r(x_batch)
                    batch_loss = mse_loss(y_pred_batch,y_batch)

                grads = tape.gradient(batch_loss,l_r.variables)

                for g,v in zip(grads,l_r.variables):
                    v.assign_sub(lr*g)
        ...
        lin_reg_export = ExportModule(model=l_r, norm_x=norm_x, norm_y=norm_y)
